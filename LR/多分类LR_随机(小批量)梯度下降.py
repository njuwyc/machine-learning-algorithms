...
def LR_SGD(dataarr,labelmat,maxiter,batch_size,alpha): #在线算法，有新输入来时可直接做，事先不需知道所有的输入。
    datamat=expanddata(dataarr)
...
def LR_SGD_2(dataarr,labelmat,maxiter): #在线算法，有新输入来时可直接做，事先不需知道所有的输入。
    datamat=expanddata(dataarr)
    yangnum,feanum=np.shape(datamat)
    weight=np.ones(feanum)
    for t in range(maxiter):
        ...
        for i in range(yangnum): #每次更新只用一个随机的样本，而不是每次都把整个数据集都做一遍
            alpha=4/(1.0+t+i)+0.01  #每次步长有变化#每次不放回随机拿出一个样本
            h=sigmoid(np.dot(datamat[index_list[i]],weight))
            error=h-labelmat[index_list[i]]
            weight=weight-alpha*error*datamat[index_list[i]]
    return weight
...
def LR_fit(X,y,maxiter,batch_size,alpha):
    yset=list(set(y))
    #print(yset)
    weight_all_time={}
    for label in yset:
        ...
        for i in range(len(y)):
            if y[i]!=label:
                y_current[i]=0
        #print(y_current)
        weight_current=LR_SGD(X,y_current,maxiter,batch_size,alpha)
...
'''
page_filepath="D:/学习/大三上/机器学习/page-blocks.data"
data=loaddataset(page_filepath)
...
for batch_size in [64,128,256]:
    for alpha in [0.01,0.03,0.1]:
        ...
        for maxiter in range(10,60,20):
            print('小批量样本数为',batch_size,'学习率为',alpha,'最大迭代次数为',maxiter,'时：')
            model=LR_fit(X_train,y_train,maxiter=maxiter,batch_size=batch_size,alpha=alpha)
            ...
            if test_accuracy>best_test_accuracy:
                ...
                best_model=[batch_size,alpha,maxiter,train_accuracy,test_accuracy]
        ...
        maxiter_all_figure.append(list(range(10,500,20)))
...
for i in range(3):
    for j in range(3):
        axes[i,j].plot(np.array(maxiter_all_figure[fig_num]),np.array(train_accuracy_all_figure[fig_num]))
        axes[i,j].plot(np.array(maxiter_all_figure[fig_num]),np.array(test_accuracy_all_figure[fig_num]))
        fig_num+=1
...
for label in yset:
    ...
    for i in range(len(y_train)):
        if y_train[i]!=label:
            y_current[i]=0
    #print(y_current)
    weight_current=LR_SGD(X_train,y_current,300)
